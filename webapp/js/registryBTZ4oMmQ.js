...
**Both ends of the size range. Not optional.**

- Apps run in the Kindredly mobile app, in a narrow side pane, and in a wide desktop window.
  Design for 320px first, then cap the wide end.
- Cap it with \`knd-wrap\` (centred, 42rem) or Pico's \`<main class="container">\`, or six list items
  become a line of text 1400px across. Canvas, games and drawing surfaces may fill the width.
...
- Re-read your CSS for fixed widths and hover-only controls before finishing.
...
`,S="# Storage decision guide\n\nAnything the user would be upset to lose is persisted. **`knd.store` is the whole answer for almost every app.**\n\n```js\n// Defaults are usable immediately, so the first render is never a blank pane.\nconst store = knd.store('chores', { items: [], streak: 0 })\n\nstore.onChange(render)          // fires when the saved data arrives, and after every change\nrender(store.data)              // render now, with defaults\n\nfunction addItem(text) {\n  store.set({ items: [...store.data.items, { text, done: false }] })   // merges and saves\n}\n```\n\nIt renders defaults first and hydrates in the background, saves after every `set`, serializes\noverlapping writes, and **never throws** — a failure leaves the app working, unsaved, with the\nreason on `store.error`. Defaults fill in any key the saved value predates.\n\n- `store.data` — live state. Read it freely; change it through `set`.\n- `store.set(patch)` — merge and save (debounced).\n- `store.save()` — write now; resolves true/false, never rejects.\n- `store.ready` — resolves once the saved value arrives. Rarely needed; `onChange` is enough.\n- `store.reset()` — back to defaults. `store.error` — last failure, or null.\n- `knd.sharedStore(...)` — identical but server-backed: syncs across devices and family members, prompts on first use.\n\n## Re-rendering without losing the caret\n\n`onChange` fires while the user may be typing — after any `set`, and again when the saved value\narrives. `app.innerHTML = html` destroys the field they are in and the caret jumps out mid-word.\n\n```js\nknd.paint(app, html)            // same thing, but focus and the caret survive it\n```\n\n**Never copy what they are typing into the store on each keystroke.** The `<input>` already holds\nit; read it when they submit.\n\n## When to use something else\n\n1. **Plain variables** for state that can reset each run — the current screen, the in-progress game.\n2. **`kindredly.refState.*`** (getdocs: refstate) when you need several independent keys, a key list,\n   or a delete. `knd.store` is one namespace and one state object.\n3. **`kindredly.sharedRefState.*`** (getdocs: sharedrefstate) for the same, server-backed.\n\nNOT available (opaque-origin sandbox): cookies, IndexedDB — never use them.\n\n**Never use `localStorage`/`sessionStorage` for app data.** The sandbox replaces both with in-memory\nshims; the `localStorage` mirror caps near 800KB, keeps only strings, and can be lost on an early\nwrite. Reach for them only if a library you did not write insists.\n\nValues must be JSON-serializable; keep them compact (no data-URLs or blobs).\n",x="# Kaboom.js patterns (v3000, preloaded global)\n\nStart with `knd.game` — never `kaboom({...})` directly. `width`/`height` are the GAME's coordinate\nsystem, not a pixel size: the canvas fills the pane on a phone and is centred and capped on a\ndesktop, and `k.width()` stays 640 everywhere.\n\n```js\nconst k = knd.game({ width: 640, height: 400, background: [24, 26, 36] })\n```\n\n**Everything you `k.add()` outside a scene is destroyed by the first `k.go()`.** Pick one:\nbuild the whole game at the top level with no scenes at all, or put every `k.add()` inside a\n`k.scene()` callback. A player added at the top level plus `k.go('play')` is an empty screen.\n\n```js\nk.scene('play', () => {\n  const player = k.add([k.rect(28, 28), k.pos(100, 100), k.color(91, 140, 255), k.area(), 'player'])\n  const label = k.add([k.text('Score: 0', { size: 20 }), k.pos(12, 12)])\n  k.add([k.circle(8), k.pos(300, 200), k.color(255, 208, 80), k.area(), 'coin'])\n  player.onCollide('coin', (c) => { k.destroy(c) })          // both objects need k.area()\n  k.onKeyDown('left', () => player.move(-240, 0))            // move() is per-second: every frame\n  k.onUpdate(() => { /* frame logic — never while(true) */ })\n})\nk.scene('over', (score) => {\n  k.add([k.text('Game over: ' + score), k.pos(k.width() / 2 - 90, k.height() / 2)])\n  k.onKeyPress(() => k.go('play'))\n  k.onClick(() => k.go('play'))                              // a phone has no keyboard\n})\nk.go('play')\n```\n\nKeys are not input on a phone. On-screen controls go in `k.overlay`, which is already stacked over\nthe canvas — appending them to `#app` puts them below the game instead of on it:\n\n```js\nconst left = document.createElement('button')\nleft.className = 'knd-pad'\nleft.textContent = '◀'\nleft.setAttribute('aria-label', 'Move left')\nleft.addEventListener('pointerdown', (e) => { e.preventDefault(); held.left = true })\nleft.addEventListener('pointerup', () => { held.left = false })\nk.overlay.append(left, right)   // the overlay spreads its children to the bottom corners\n```\n\nOther pieces: `k.wait(1, fn)` / `k.loop(2, fn)` for timers; `k.body()` (+`k.area()`) for gravity,\na floor needs `k.body({ isStatic: true })`, `player.jump(600)`.\n\nKeep the score in `knd.store` (getdocs: storage) so a best score survives a reload.\n\nNo external sprite/sound URLs (the sandbox has no asset network) — use `k.rect`/`k.circle`/`k.text`\nprimitives. Colors drawn inside the canvas are literal RGB and that is fine.\n",A='# Alpine.js patterns (preloaded)\n\nAlpine gives reactive UI without a render loop — good for forms and small interactive widgets:\n\n```js\nconst app = document.getElementById(\'app\')\napp.innerHTML = `\n  <main class="container" x-data="{ count: 0, name: \'\' }">\n    <h3>Counter</h3>\n    <p>Count: <strong x-text="count"></strong></p>\n    <button @click="count++">+1</button>\n    <input x-model="name" placeholder="Your name" />\n    <p x-show="name">Hello, <span x-text="name"></span>!</p>\n  </main>`\n```\n\nAlpine 3 auto-initializes DOM injected after load (it watches the document). If directives ever fail to activate, call `Alpine.initTree(app)` once after setting innerHTML — but do not call it on trees Alpine already initialized (duplicate handlers).\n\nCore directives:\n- `x-data="{...}"` — component state (on a wrapping element)\n- `x-text` / `x-show` / `x-if` (on `<template>`) — bind text / visibility / existence\n- `x-model` — two-way bind inputs\n- `@click`, `@submit.prevent` — events\n- `x-for` (on `<template>`) — lists: `<template x-for="item in items" :key="item.id">`\n\nBigger state with methods:\n\n```html\n<div x-data="{\n  items: [],\n  add(text) { if (text.trim()) this.items.push({ text, done: false }) }\n}">\n```\n\nChoose ONE approach per app: either Alpine drives the UI (declarative), or a manual `render()` rebuilds innerHTML — mixing both causes lost state (innerHTML replacement destroys Alpine components). Alpine + async host APIs: call them from `@click` handlers and set state properties when they resolve; Alpine re-renders automatically.\n',L=`# Location API (capability-gated device position)
...
\`\`\`
`,T=`# Capture API (capability-gated photo capture)
...
\`\`\`
`,I="'__KND_THEME_CSS__'",P=`// Starter game (Kaboom.js). Arrows or the on-screen pads move the square; collect the dots.
(() => {
  const app = document.getElementById('app')
  if (!app) return
  ...
  // knd.game, never kaboom() directly: width/height are the GAME's coordinates, and the canvas
  // fits the pane on a phone and is centred on a desktop.
  const k = knd.game({ width: 640, height: 400, background: [24, 26, 36] })
  ...
  // A phone has no arrow keys, so the same movement is driven by held flags that BOTH
  // the keyboard and the on-screen pads set.
  const held = { left: false, right: false, up: false, down: false }
  const pad = (label, dir) => {
    const btn = document.createElement('button')
    btn.className = 'knd-pad'
    btn.textContent = label
    btn.setAttribute('aria-label', dir)
    btn.addEventListener('pointerdown', (e) => { e.preventDefault(); held[dir] = true })
  ...
  // k.overlay is stacked over the canvas already; appending to #app would put these below it.
  k.overlay.append(pad('◀', 'left'), pad('▶', 'right'))

  k.onKeyDown('left', () => { held.left = true })
  k.onKeyDown('right', () => { held.right = true })
  k.onKeyDown('up', () => { held.up = true })
  k.onKeyDown('down', () => { held.down = true })
  k.onKeyRelease(() => { held.left = held.right = held.up = held.down = false })

  k.onUpdate(() => {
    if (held.left) player.move(-SPEED, 0)
    if (held.right) player.move(SPEED, 0)
    if (held.up) player.move(0, -SPEED)
    if (held.down) player.move(0, SPEED)
...
`,C=`// Starter tracker: add, check off, and remove items. Persists via knd.store.
(() => {
  const app = document.getElementById('app')
  if (!app) return
  ...
  // One call: defaults render immediately, the saved value arrives in the background, and every
  // set() is written. No load/save/try-catch to hand-write. See getdocs: storage.
  const store = knd.store('tracker', { items: [] })

  const render = () => {
    const items = store.data.items
    app.innerHTML =
      // knd-wrap caps the column on a wide window; no title heading, because Kindredly already
      // shows the app's name around this pane.
      '<main class="knd-wrap knd-stack">'
      + '<form id="addForm" role="group">'
      + '  <input id="newItem" placeholder="Add an item…" autocomplete="off" />'
      + '  <button type="submit">Add</button>'
      + '</form>'
      + (items.length === 0
          ? '<div class="knd-empty"><knd-icon name="check2-square" size="32"></knd-icon><p class="knd-muted">Nothing yet — add your first item above.</p></div>'
          : '<ul class="knd-list">'
            + items.map((it, i) =>
                '<li class="knd-list-item">'
                + '<input type="checkbox" data-toggle="' + i + '"' + (it.done ? ' checked' : '') + ' />'
                + '<span class="knd-grow' + (it.done ? ' knd-muted' : '') + '">' + escapeHtml(it.text) + '</span>'
                + '<button class="knd-icon-btn" data-remove="' + i + '" aria-label="Remove">' + (kindredly.icons?.svg('x-lg', { size: 14 }) || 'Remove') + '</button>'
                + '</li>'
              ).join('')
            + '</ul>')
      + '</main>'
    ...
    document.getElementById('addForm')?.addEventListener('submit', (e) => {
      ...
      if (!text) return
      store.set({ items: [...store.data.items, { text, done: false }] })
    })
    app.querySelectorAll('[data-toggle]').forEach(el => el.addEventListener('change', () => {
      const i = Number(el.getAttribute('data-toggle'))
      const next = store.data.items.map((it, idx) => (idx === i ? { ...it, done: !it.done } : it))
      store.set({ items: next })
    }))
    app.querySelectorAll('[data-remove]').forEach(el => el.addEventListener('click', () => {
      const i = Number(el.getAttribute('data-remove'))
      store.set({ items: store.data.items.filter((_, idx) => idx !== i) })
    }))
  }

  // onChange fires when the saved data arrives and after every set — one re-render path.
...
`)}];function G(e){return s.find(t=>t.id===e)||s[0]}const J=[{id:"game",label:"Game",icon:"controller",blurb:"Arcade-style games with Kaboom.js",packText:String(c),scaffoldId:"game"},{id:"tool",label:"Tool",icon:"check2-square",blurb:"Trackers, lists, counters, planners",packText:String(p),scaffoldId:"tool"},{id:"content",label:"Viewer",icon:"journal-text",blurb:"Look up and display content from the web",packText:String(h),scaffoldId:"content"},{id:"creative",label:"Creative",icon:"palette",blurb:"Drawing and creative canvas apps",packText:String(u),scaffoldId:"creative"}],V=String(m),o=[{id:"refstate",title:"Ref State API",oneLiner:"local persistence, no prompts",text:String(f)},{id:"sharedrefstate",title:"Shared Ref State API",oneLiner:"server-backed persistence, syncs, permissioned",text:String(g)},{id:"net",title:"Network API",oneLiner:"fetch(), permissioned per host and per read/write",text:String(y)},{id:"files",title:"Files API",oneLiner:"read this app's attachments",text:String(k)},{id:"theming",title:"Theming",oneLiner:"full --knd-* token table and light/dark rules",text:String(b)},{id:"icons",title:"Icon set",oneLiner:"built-in SVG icons and the complete name list",text:String(w)},{id:"ui-kit",title:"UI kit",oneLiner:"knd-* CSS classes for cards, layout, buttons, chips",text:String(v)},{id:"ui-design",title:"UI design rules",oneLiner:"what a finished app looks like; the mistakes to avoid",text:String(E)},{id:"storage",title:"Storage guide",oneLiner:"refState vs sharedRefState, and why not localStorage",text:String(S)},{id:"kaboom-patterns",title:"Kaboom.js patterns",oneLiner:"game objects, input, collisions, scenes",text:String(x)},{id:"alpine-patterns",title:"Alpine.js patterns",oneLiner:"reactive forms and widgets",text:String(A)},{id:"location",title:"Location API",oneLiner:"device position; guardian-gated for kids; no-internet rule",text:String(L)},{id:"capture",title:"Capture API",oneLiner:"photo capture via the host camera screen",text:String(T)}];function r(e){return e&&J.find(t=>t.id===e)||null}function W(e){var t;return((t=r(e))==null?void 0:t.packText)||V}function Y(e){const t=String(e||"").trim().toLowerCase();return o.find(n=>n.id===t)||null}function X(){return o.map(e=>e.id)}function Q(e,t){const n=r(e);if(!n)return[];const a=D.find(d=>d.id===n.scaffoldId);if(!a)return[];const i=G(t),l=a.code.replace(I,JSON.stringify(i.css));return[{filename:a.filename,code:l}]}export{J as A,q as D,s as a,W as b,Q as c,Y as g,X as l};
