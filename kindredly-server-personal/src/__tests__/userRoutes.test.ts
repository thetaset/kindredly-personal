...
import App from '../app';
import {ItemTypeEnum, LoginType, UserType} from 'tset-sharedlib/shared.types';
...
describe('User Routes', () => {
  ...
  test('Create User', async () => {
    ...
    jest.spyOn(UserRepo.prototype, 'where').mockReturnValue({
      first() {
        return null;
      },
    } as any);
    ...
    const response = await request(app).post('/v3.0/auth/register').send({
      username: 'mytestuser',
      email: 'testuser@example.com',
      type: user.type,
