...
import {checkEmail, checkUserName} from '@/utils/user.utils';
...
class OpenAuthValidatorService implements AuthValidatorService {
  ...
  async validateUserCredentials(userData: CreateUserDto): Promise<{user: User; verified: boolean}> {
    ...
    let verified = false;
    if (userData.loginType == LoginType.internal) {
      if (!userData.password) throw new HttpException(400, 'Password is missing');

      user = await this.users.findByUsername(userData.username.toLowerCase());
      if (!user) {
        user = await this.users.findByEmail(userData.email || userData.username);
      }
